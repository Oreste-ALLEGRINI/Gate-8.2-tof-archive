...
/** Modif Oreste **/
//------------------------------------------------------------------------
/*void GateSourceOfPromptGammaData::InitializeToF()
{
    unsigned int sizeX = mImage->GetResolution().x();
    unsigned int sizeY = mImage->GetResolution().y();
    unsigned int sizeZ = mImage->GetResolution().z();
    ...
    double timeStep  = (mImage->GetMaxValue()-mImage->GetMinValue())/nbOfBins;
    double time = 0.0;
    long index_image = 0;
    long index_data = 0;
    float * data = mImage->GetDataFloatPointer();
      ...
      for(unsigned int j=0; j<sizeY; j++) {
        for(unsigned int i=0; i<sizeX; i++) {
          if (mDataCounts[index_image] == 0) { // FIXME
            ...
            time = mImage->GetMinValue();
            mTimeGen[index_image] = new TH1D;
            // This is much much faster to use the constructor without
            // param, the SetBins than using the following line with constructor :
            // new TH1D("", "", nbOfBins, mImage->GetMinValue(), mImage->GetMaxValue());
            TH1D * h = mTimeGen[index_image];
            h->SetBins(nbOfBins, mImage->GetMinValue(), mImage->GetMaxValue());
            for(unsigned int l=0; l<nbOfBins; l++) {
              h->Fill(time, data[index_data]);
              index_data++;
              time += timeStep;
            }
            nbNonZero++;
...
/* void GateSourceOfPromptGammaData::SampleRandomPositionTime(G4ThreeVector & position, G4double & tof)
{
  ...
  double s ;
  int a = 0;
  tofdata.open("/Users/allegrini/Documents/5.Simulations/vpgTLEtof/Output_and_Results_vpgTLEtof/tof.dat");
